...
class Stamp extends \Core\Controller
{
                ...
                if( !isset($_FILES) ){
                    ...
                    if(!empty($_FILES['photo']['name'])){  // upload exists
                        ...
                        if(move_uploaded_file($_FILES['photo']['tmp_name'], $uploadfile)){
                            ...
                            if (\App\Models\Stamp::insertPhoto($st_id, $photo_name, 1)) echo "<script>location.href='".\App\Config::URL_RACINE."/user/profil';</script>";
                        }else{
    ...
    public function modifierAction()
    {
        ...
        if (!isset($_SESSION['user'])){
            ...
            $photos = \App\Models\Stamp::getPhoto($st_id);
            ...
            if (!empty($_POST)) {
                ...
                if( empty($_FILES['photo']['name']) ){    // no photo uploaded, do not change the photo
                    ...
                    if (\App\Models\Stamp::modifier($_POST)) echo "<script>location.href='".\App\Config::URL_RACINE."/user/profil';</script>";
                    ...
                    //treatment of image upload
                    if($_FILES['photo']['error'] && ($_FILES['photo']['error'] != 4)){
                    ...
                    if(move_uploaded_file($_FILES['photo']['tmp_name'], $uploadfile)){
                        ...
                        //insert into the table `photo`
                        if (\App\Models\Stamp::modifierPhoto($st_id, $photo_name)) echo "<script>location.href='".\App\Config::URL_RACINE."/user/profil';</script>";
                    }else{
                        echo "<script>alert('image non trouvé');history.back();</script>";
                        exit;
                    }
    ...
    public function photoAction()
    {
        ...
        if (!isset($_SESSION['user'])){
                ...
                if( !isset($_FILES) ){
                    ...
                    if(!empty($_FILES['photo']['name'])){  // upload exists
                        ...
                        if(move_uploaded_file($_FILES['photo']['tmp_name'], $uploadfile)){
                            ...
                            //insert into the table `photo`
                            if (\App\Models\Stamp::insertPhoto($st_id, $photo_name, 0)) echo "<script>location.href='".\App\Config::URL_RACINE."/user/profil';</script>";
                        }else{
                            echo "<script>alert('image non trouvé');history.back();</script>";
                            exit;
                        }
            ...
            View::renderTemplate('Stamp/photo.html',
                                ['st_id' => $st_id,
                                 'user' => $user]
            );
        }
    ...
    public function unphotoAction()
    {
        ...
        if (!isset($_SESSION['user'])){
            ...
            $id = $this->route_params['id'];
            if (\App\Models\Stamp::deletePhoto($id)) echo "<script>history.back();</script>";
        }
    }
    ...
    public function supprimerAction()
    {
        ...
        if (!isset($_SESSION['user'])){
            ...
            $id = $this->route_params['id'];
            if (\App\Models\Stamp::delete($id)) echo "<script>location.href='".\App\Config::URL_RACINE."/user/profil';</script>";
        }
    }
}
